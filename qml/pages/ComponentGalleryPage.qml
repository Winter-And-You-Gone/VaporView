...
Item {
    ...
    function sliderCenter(prop, fb) { if (styleSliderCenters && styleSliderCenters[prop] !== undefined) return Number(styleSliderCenters[prop]); return Number(fb) }
    function recenterStyleSliders() { var n = {}; for (var i=0; i<styleSliderProps.length; ++i) { var p=styleSliderProps[i]; n[p]=Math.round(Number(appBackend[p])) }; styleSliderCenters = n }
