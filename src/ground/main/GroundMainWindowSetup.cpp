...
void MainWindow::setupStatusBar()
{
    ...
    state_->status_label_ = new QLabel(this);
    statusBar()->addWidget(state_->status_label_);

    state_->status_task_progress_bar_ = new QProgressBar(this);
    state_->status_task_progress_bar_->setVisible(false);
    state_->status_task_progress_bar_->setTextVisible(true);
    state_->status_task_progress_bar_->setFixedHeight(18);
    state_->status_task_progress_bar_->setMinimumWidth(180);
    state_->status_task_progress_bar_->setMaximumWidth(260);
    state_->status_task_progress_bar_->setRange(0, 100);
    state_->status_task_progress_bar_->setValue(0);
    state_->status_task_progress_bar_->setFormat(QString());
    statusBar()->addWidget(state_->status_task_progress_bar_);

    state_->status_task_spinner_label_ = new QLabel(this);
    state_->status_task_spinner_label_->setObjectName("statusTaskSpinner");
    state_->status_task_spinner_label_->setAlignment(Qt::AlignCenter);
    state_->status_task_spinner_label_->setFixedSize(18, 18);
    state_->status_task_spinner_label_->setVisible(false);
    statusBar()->addWidget(state_->status_task_spinner_label_);

    state_->status_task_spinner_timer_ = new QTimer(this);
    state_->status_task_spinner_timer_->setInterval(120);
    connect(state_->status_task_spinner_timer_, &QTimer::timeout, this, [this]() {
        if (!state_->status_task_spinner_label_ || !state_->status_task_spinner_label_->isVisible())
        {
            return;
        }
        ...
        state_->status_task_spinner_label_->setText(frames.at(state_->status_task_spinner_index_ % frames.size()));
        state_->status_task_spinner_index_ = (state_->status_task_spinner_index_ + 1) % frames.size();
    });
...
{
    if (!state_->status_task_spinner_label_ || !state_->status_task_spinner_timer_)
    {
        return;
    }

    state_->status_task_spinner_label_->setVisible(true);
    if (!state_->status_task_spinner_timer_->isActive())
    {
        state_->status_task_spinner_index_ = 0;
        state_->status_task_spinner_label_->setText(QStringLiteral("◐"));
        state_->status_task_spinner_timer_->start();
    }
}

void MainWindow::stopStatusTaskSpinner()
{
    if (state_->status_task_spinner_timer_)
    {
        state_->status_task_spinner_timer_->stop();
    }
    if (state_->status_task_spinner_label_)
    {
        state_->status_task_spinner_label_->clear();
        state_->status_task_spinner_label_->setToolTip(QString());
        state_->status_task_spinner_label_->setVisible(false);
    }
    state_->status_task_spinner_index_ = 0;
}
...
{
    if (!state_->status_task_progress_bar_)
    {
        return;
    }
    ...
    state_->status_task_progress_bar_->setVisible(true);
    state_->status_task_progress_bar_->setRange(0, normalizedMaximum);
    state_->status_task_progress_bar_->setValue(normalizedValue);
    state_->status_task_progress_bar_->setFormat(QStringLiteral("%1 %p%").arg(label));
    state_->status_task_progress_bar_->setToolTip(label);
    if (state_->status_task_spinner_label_)
    {
        state_->status_task_spinner_label_->setToolTip(label);
    }
...
void MainWindow::showBusyStatusTaskProgress(const QString& label)
{
    ...
    state_->status_task_progress_bar_->setRange(0, 100);
    state_->status_task_progress_bar_->setValue(0);
    state_->status_task_progress_bar_->setFormat(label);
    state_->status_task_progress_bar_->setToolTip(label);
    if (state_->status_task_spinner_label_)
    {
        state_->status_task_spinner_label_->setToolTip(label);
...
void MainWindow::hideStatusTaskProgress()
{
    ...
    state_->status_task_progress_bar_->setValue(0);
    state_->status_task_progress_bar_->setFormat(QString());
    state_->status_task_progress_bar_->setToolTip(QString());
