...
- 所有设备尝试完成后，如果至少有一个设备成功连接，则立即创建一个新的 `session_*` 目录
- 点击“断开”、取消连接或连接流程失败后自动停止并关闭本次 session
- 每次新的连接会话都会生成一个新的独立目录，避免不同实验数据混在一起
...
- `devices.csv` 由独立记录线程按固定 `10 Hz` 写入，每行记录当前时刻的最新有效设备快照
- TCP 面板中的“归一化二次谐波”按 `10 Hz` 导出到 `.dat`
- 每帧 `.dat` 数据格式固定为 `uint64 时间戳 + 50000 个 float32`
- 波形写盘采用“采集回调 + 缓冲队列 + 独立写盘线程”结构
- 波形文件默认每 `1` 分钟滚动一个新文件，文件名采用本地时间戳，便于回查
