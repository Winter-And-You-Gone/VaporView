...
- 快照导出：把当前界面中的最新一帧数据导出为文件
- 持续记录：开始记录后，程序会在每次设备新数据到达时，按时间戳持续追加到文件中，直到手动停止
...
- CSV 持续记录会按行写入 `timestamp_utc, device, valid, error_message, payload_json`
- JSON 持续记录会写成一个包含 `records` 数组的 JSON 文件，每条记录都带时间戳、设备名、有效性和设备负载数据
- 持续记录与快照导出互不冲突，可以根据需要单独使用
