...
| 设备 | 每个设备都有独立连接 / 测试 / 详情按钮 | 旧 QWidget 对选中的串口设备使用全局连接流程，没有为每个设备提供独立测试入口；详细数据页是全局数据视图 | QML 已按 React 样式显示底部按钮组；连接按钮仍调用真实全局连接流程，TCP 卡片单独连接波形源，EPSILON 的实验按钮打开现有 EPS Rates 快捷弹窗，详情按钮进入全局详细数据页 | 等后端支持单设备长耗时采集器所有权后，再补独立测试命令。 |
