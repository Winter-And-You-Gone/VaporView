...
选择 BMP390 或 SHT45 时，设备配置页会自动把对应波特率切换为 115200；串口号仍需选择 MCU/单板机实际枚举出的端口。Ground 本地模式和 Remote Sky 配置都使用同一组 source 字段；旧 SkyConfig 缺少 source 时默认保持 PTB210/HMP3 语义。
