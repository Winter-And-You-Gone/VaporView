...
- `QComboBox` 下拉框的原生容器只做安全样式：`QAbstractItemView` stylesheet、面板圆角、上下留白、整行高亮、必要的 rounded mask。需要外侧柔和阴影且暂不迁移到 `SingleLevelPopupMenu` 时，只能使用独立的透明辅助窗口绘制“外圈阴影”，辅助窗口中心必须完全透明，不能绘制白色面板覆盖 popup 内容，也不能改变原生 popup 容器的透明背景或 backing store。启用自绘外圈阴影时必须禁用原生 popup window drop shadow，让 popup 和触发 `QComboBox` 保持清晰间距，并裁掉顶部/靠触发框侧阴影，只保留侧下方和底部柔影，避免上下边缘出现双层阴影或贴边重叠。
