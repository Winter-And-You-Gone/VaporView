...
2. 默认使用仓库根目录下的 `build` 目录进行配置和构建；如果 `build` 已存在，就在该目录上增量构建。
3. 只有在构建成功后，才允许继续提交并推送；如果构建失败，先修复问题，不要 push 失败状态的改动。
4. 确认需要提交的文件，只包含本次任务相关改动，不回滚用户自己的其它改动。
5. 使用清晰的非交互式 git 命令创建提交。
6. 将当前分支推送到 `origin`。
7. 在最终回复里说明:
   - 做了什么
   - 如何验证
